...
def GhostBottleneck(x,dwkernel,strides,out,ratio):
    y1 = GhostModule(x,out,ratio,1,3)
...
def Ghost_UNet (input_size=(128, 128, 1)):
    ...
    x1 = GhostBottleneck(x, 3, 1, 64, 2)
    pool1 = AveragePooling2D(pool_size=(2, 2))(x1)
    x2 = GhostBottleneck(pool1, 3, 1, 128, 2)
    pool2 = AveragePooling2D(pool_size=(2, 2))(x2)
    x3 = GhostBottleneck(pool2, 3, 1, 256,  2)
    pool3 = AveragePooling2D(pool_size=(2, 2))(x3)
    x4 = GhostBottleneck(pool3, 3, 1, 512,  2)
    #drop4 = Dropout(0.5)(x4)
    pool4 = AveragePooling2D(pool_size=(2, 2))(x4)
    x5 = GhostBottleneck(pool4, 3, 1, 1024, 2)
    #drop5 = Dropout(0.5)(x5)

    up6 = Conv2D(512, 2, activation='relu', padding='same', kernel_initializer='random_uniform')(
        UpSampling2D(size=(2, 2))(x5))
    merge6 = concatenate([x4, up6], axis=3)
    x6 = GhostBottleneck(merge6, 3, 1, 512,  2)

    up7 = Conv2D(256, 2, activation='relu', padding='same', kernel_initializer='random_uniform')(
        UpSampling2D(size=(2, 2))(x6))
    merge7 = concatenate([x3, up7], axis=3)
    x7 = GhostBottleneck(merge7, 3, 1, 256,  2)

    up8 = Conv2D(128, 2, activation='relu', padding='same', kernel_initializer='random_uniform')(
        UpSampling2D(size=(2, 2))(x7))
    merge8 = concatenate([x2, up8], axis=3)
    x8 = GhostBottleneck(merge8, 3, 1, 128,  2)

    up9 = Conv2D(64, 2, activation='relu', padding='same', kernel_initializer='random_uniform')(
        UpSampling2D(size=(2, 2))(x8))
    merge9 = concatenate([x1, up9], axis=3)
    x9 = GhostBottleneck(merge9, 3, 1, 64,  2)
